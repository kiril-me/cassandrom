...
function init(self, obj, doc, prefix) {
  ...
  function _init(index) {
    i = keys[index];
    key = i;
    if(self.schema.options.insensitive) {
      key = key.toLowerCase();
    }
    path = prefix + key;
    ...
    if (!schema && utils.isObject(obj[i]) &&
      ...
      if(schema && schema.path.indexOf('.') > -1) {
        var parts = schema.path.split('.');
        var el = doc;
        ...
        if (schema) {
            try {
              el[nestedKey] = schema.cast(obj[i], self, true);
            } catch (e) {
        ...
        if (schema) {
          try {
            doc[schema.path] = schema.cast(obj[i], self, true);
          } catch (e) {
            self.invalidate(e.path, new ValidatorError({
              path: e.path,
              message: e.message,
