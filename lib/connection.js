...
Connection.prototype.model = function (name, schema, collection, skipInit, cb) {
  ...
  var model = this.cassandrom.model(name, schema, collection, skipInit);
  model.base = model.prototype.base = this;

  if(this._options.createTables) {
    this.createTableIfNotExists(name, model.schema, function(error) {
      if(error) {
...
Connection.prototype.createTableIfNotExists = function(name, schema, cb) {
  if(schema._tableCreated) {
    ...
    var self = this;
    this.execute('SELECT table_name FROM system_schema.tables WHERE keyspace_name=?', [name], function(error, result) {
      if(error) {
