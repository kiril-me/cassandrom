...
var OPERATIONS = {
  '$in': function(path, condition, params) {
    var line = path.options.name + ' IN (';
    for(var o = 0; o < condition.length; o++) {
      if(o > 0) {
        line += ', ';
      }
      line += '?';
      params.push(path.castForQuery( condition[o] ) );
    }
    line += ')';
    return line;
  },
  '$gt': function(path, condition, params) {
    params.push(path.castForQuery( condition ) );
    return path.options.name + ' > ?';
  },
  '$lt':  function(path, condition, params) {
    params.push(path.castForQuery( condition ) );
    return path.options.name + ' < ?';
  },
};
...
function Query(conditions, model, options) {
  ...
  if (this.schema) {
    ...
    this._distinct = this.model.hooks.createWrapper('distinct',
        Query.prototype._distinct, this, kareemOptions);
    this._find = this.model.hooks.createWrapper('find',
        Query.prototype._find, this, kareemOptions);
    this._update = this.model.hooks.createWrapper('update',
        Query.prototype._update, this, kareemOptions);
...
Query.prototype._count = function(callback) {
  ...
  var cb = function(err, docs) {
    ...
    _this.__parseDataOneValue('count', docs, false, callback);
  };
  this._select(['count(*)'], this._conditions, 1, null, cb);
};

Query.prototype._update = function(callback) {
...
Query.prototype._find = function(callback) {
  ...
  var cb = function(err, docs) {
    ...
    if (_this.options.populate) {
      var pop = utils.object.vals( _this.options.populate );
      _this.__parseData(fields, docs, function(error, docs) {
        if(error) {
          return callback(err);
        }
      ...
      /*
      _this.model.populate(docs.rows, pop, function(err, dd) {
        if (err) {
          return callback(err);
        }
  ...
  if(conditions) {
    ...
    for(var p in conditions) {
      ...
      if(path) {
        ...
        cond = conditions[p];
        if(cond.constructor.name !== 'Uuid' && utils.isObject(cond)) {
          var next = false;
          for(var op in cond) {
            if(OPERATIONS[op]) {
              if(next) {
              ...
              where += OPERATIONS[op](path, cond[op], params);
            } else {
          ...
          params.push(path.castForQuery( cond ));
        }
      }
    }
...
Query.prototype._findOne = function(callback) {
  ...
  var cb = function(err, docs) {
    ...
    if (_this.options.populate) {
      var pop = utils.object.vals( _this.options.populate );
      _this.__parseDataOne(fields, docs, function(error, doc) {
        if(error) {
          return callback(err);
        }
        _this.model.populate(doc, pop, callback);
      });
/*
      var pop = utils.object.vals( _this.options.populate );
      _this.model.populate(docs, pop, function(err, docs) {
...
Query.prototype.distinct = function(field, conditions, callback) {
  ...
  this.op = 'distinct';
  if (callback) {
    this._distinct(field, callback);
  }

  return this;
};

Query.prototype._distinct = function(field, callback) {
  var _this = this;
  var cb = function(err, docs) {
    if (err) {
      return callback(err);
    }
    _this.__parseDataOneValue(field, docs, true, callback);
  };

  this._select(['DISTINCT ' + field], this._conditions, this._limit, this._sort, cb);
...
Query.prototype._getFields = function() {
  if(this._fields) {
    if(utils.isObject(this._fields)) {
      var fields = [];
      for(var p in this._fields) {
        if(p[0] === '_') {
          fields.push(p.substring(1));
        } else {
          fields.push(p);
        }
      }
  ...
  if (callback) {
    promise.then(
      ...
      function(error) {
        ...
        // If we made it here, we must have an error in the callback, so we need to emit.
...
Query.prototype.__parseDataOneValue = function(field, docs, array, callback) {
  var data = null;
  if(docs.rows && docs.rows.length > 0) {
    if(array) {
      data = [];
      data.length = docs.rows.length;
      for(var i = 0; i < docs.rows.length; i++) {
        data[i] = docs.rows[i][field];
      }
    } else {
      data = docs.rows[0][field];
    }
  }
  callback(null, data);
  return data;
