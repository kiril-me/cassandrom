...
Model.init = function init () {
  if ((this.schema.options.autoIndex) ||
      (this.schema.options.autoIndex === null && this.base.config.autoIndex)) {
...
function populate(model, docs, options, callback) {
  ...
  for (i = 0; i < len; i++) {
    ...
    hasOne = true;
    if (mod.foreignField !== 'id' || !match['id']) {
      if(ids.length === 1) {
        match[mod.foreignField] = ids[0];
      } else {
...
function getModelsMapForPopulate(model, docs, options) {
  ...
  var optionPath = options.path;
  refPath = schema && schema.options && schema.options.refPath;
  if(model.schema.options.insensitive) {
    //refPath = refPath && refPath.toLowerCase();
    //discriminatorKey = discriminatorKey && discriminatorKey.toLowerCase();
  ...
  for (i = 0; i < len; i++) {
      ...
      if (!modelNameFromQuery) {
        ...
        if (!schema && discriminatorKey) {
          ...
          if (modelForFindSchema) {
            modelForCurrentDoc = model.base.model(modelForFindSchema);
            schemaForCurrentDoc = modelForCurrentDoc._getSchema(optionPath);
        ...
        var virtual = modelForCurrentDoc.schema._getVirtual(optionPath);
    ...
    virtual = model.schema._getVirtual(optionPath);
    ...
    var foreignField = virtual && virtual.options ?
      virtual.options.foreignField : 'id';
    ...
    if (doc.$__) {
      doc.populated(optionPath, options._docs[id], options);
    }
