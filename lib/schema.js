...
Schema.prototype._insertValue = function(path, obj, name, fields, params, validationError) {
  ...
  if(namePath) {
    namePath.doValidate(value, function (error) {
      if(error) {
        if (!validationError) {
    ...
    if(fields.length === 0 || (fields.length > 0 && fields.indexOf(path) >= 0)) {
        value = namePath.castForQuery( value );
...
*/
  ...
  var newKey = this.underlinepaths[key];
  if(newKey) {
   key = newKey;
  }
  ...
  // get path
  if (obj == undefined) {
    if(this.insensitive[key]) {
      key = this.insensitive[key];
    }
    ...
    if (this.subpaths[key]) {
      return this.subpaths[key];
    }

    if (this.singleNestedPaths[key]) {
      return this.singleNestedPaths[key];
    }

    // subpaths?
    return /\.\d+\.?.*$/.test(key)
      ? getPositionalPath(this, key)
      : undefined;
  }
  ...
  this.paths[key] = Schema.interpretAsType(this.options.insensitive, path, obj);
  this.insensitive[key.toLowerCase()] = key;

  if(underlinepath) {
    underlinepath += '_' + last;
    this.underlinepaths[underlinepath] = key;
  }
