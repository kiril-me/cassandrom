...
describe('model: querying:', function() {
  ...
  before(function() {
    ...
    Comments.add({
      blogId: { type: cassandrom.UUIDType, default: cassandrom.uuid },
      title: String,
      date: Date,
      body: String,
      //comments: [ Comments ]
    });
    Comments.setPrimaryKey('blogId');
    ...
    BlogPostB.index({ title: 1, slug: 1, score: 1 });

    cassandrom.model('BlogPostB', BlogPostB);
    ...
    ModSchema.setPrimaryKey('id');
    cassandrom.model('Mod', ModSchema);

    geoSchema = new Schema({
      id: { type: cassandrom.UUIDType, default: cassandrom.uuid, primaryKey: true },
  ...
  before(function(done) {
    ...
    db = start(function(error, result) {
      if(error) {
        ...
        BlogPostB = db.model('BlogPostB', collection, function(error) {
          if(error) {
            done(error);
          } else {
            Address = db.model('Address', Address, 'addresses_' + random(), done);
          }
        });
      }
    });
  ...
  it('a query is executed when a callback is passed', function(done) {
    var count = 5,
        q = {id: new cassandrom.uuid}; // make sure the query is fast
      ...
      post.save(function(err) {
        ...
        post.save(function(err) {
          ...
          BlogPostB.count({ /* title: title */}, function(err, count) {
            ...
            assert.equal(count.constructor.name, 'Long');
  ...
  describe('distinct', function() {
    ...
    it('executes when you pass a callback', function(done) {
      Address.create({zip: '10010', name: 'Bank'}, {zip: '10010', name: 'Grocery'}, {zip: '99701', name: 'Bank'}, function(err) {
    ...
    it('permits excluding conditions', function(done) {
      Address.create({zip: '10010', name: 'Bank'}, {zip: '10010', name: 'Grocery'}, {zip: '99701', name: 'Bank'}, function(err) {
        assert.ifError(err);
  ...
  describe('findOne', function() {
    it('works', function(done) {
      ...
      post.save(function(err) {
        ...
        BlogPostB.findOne({id: post.id}, function(err, doc) {
          assert.ifError(err);
    ...
    it('casts $modifiers', function(done) {
      ...
      post.save(function(err) {
        ...
        var after = cassandrom.uuid();
        var query = {'id': { $gt: after, $lt: before}};
...
/*
    ...
    it('based on embedded doc fields', function(done) {
    ...
    it('works with nested docs and string ids', function(done) {
    ...
    it('regex with Array', function(done) {
...
/*
  ...
  describe('find', function() {
    ...
    it('where $exists', function(done) {
      ...
      cassandrom.model('Exists', ExistsSchema);
    ...
    describe('text search indexes', function() {
      it('works with text search ensure indexes ', function(done) {
        ...
        blogPost.collection.ensureIndex({title: 'text'}, function(error) {
          ...
          var a = new blogPost({title: 'querying in cassandrom'});
          var b = new blogPost({title: 'text search in cassandrom'});
          a.save(function(error) {
            ...
            b.save(function(error) {
              ...
              exec(function(error, documents) {
                ...
                assert.equal(documents[0].title, 'text search in cassandrom');
  ...
  describe('sort', function() {
    ...
    it('handles sorting by text score', function(done) {
      ...
      blogPost.collection.ensureIndex({title: 'text'}, function(error) {
        assert.ifError(error);
        var a = new blogPost({title: 'searching in cassandrom'});
        var b = new blogPost({title: 'text search in cassandrom'});
        a.save(function(error) {
          assert.ifError(error);
          b.save(function(error) {
            ...
            exec(function(error, documents) {
              ...
              assert.equal(documents[0].title, 'text search in cassandrom');
              assert.equal(documents[1].title, 'searching in cassandrom');
              db.close();
              done();
            });
          });
  ...
  describe('lean', function() {
    it('find', function(done) {
      ...
      post.save(function(err) {
        ...
        BlogPostB.find({title: title}).lean().exec(function(err, docs) {
          assert.ifError(err);
          assert.equal(docs.length, 1);
          assert.strictEqual(docs[0] instanceof cassandrom.Document, false);
    ...
    it('findOne', function(done) {
      ...
      post.save(function(err) {
        ...
        BlogPostB.findOne({title: title}, null, {lean: true}, function(err, doc) {
          ...
          assert.ifError(err);
          assert.ok(doc);
          assert.strictEqual(false, doc instanceof cassandrom.Document);
          done();
        });
      });
    });
    it('properly casts nested and/or queries (gh-676)', function(done) {
      ...
      var M = cassandrom.model('andor' + random(), sch);
